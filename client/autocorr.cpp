...
//  static bool acCpuDone = false;
//  FILE *acfilec;
//  if (!acCpuDone) acfilec = fopen("ac_cpu.txt","w");
  ...
  for(int b = 0; b < len/blksize; b++) {
      partial = 0.0f;
	  for(i = 0; i < blksize; i++) {
//		if (!acCpuDone) fprintf(acfilec,"cpu[%10d], %10.6f\n", b*blksize+i,AutoCorrelation[b*blksize+i]);
...
//  if (!acCpuDone) { fclose(acfilec); acCpuDone = true; }
...
//  fprintf(stderr,"CPU autcorr peak power %10.6f\n",ai.a.peak_power);
  return 0;
}
