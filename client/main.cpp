...
int main(int argc, char** argv) {
  ...
  for (i=1; i<argc; i++) {
    ...
    } else if (!strncmp(p, "nocuda", 6)) {
		bNoCUDA = true;
    } else if (!strncmp(p, "usecudadev", 10)) {
		gCUDADevPref = atoi(argv[++i]);
    } else if (!strncmp(p, "device", 6)) {
		gCUDADevPref = atoi(argv[++i])+1;
    } else if (!strncmp(p, "logperf", 7)) {
		bLogPerf2StdErr = true;
    } else if (!strncmp(p, "poll", 4)) {
		bPollCUDA = true;
    }
#ifdef _WIN32
//X-Branch command line extensions for global Cuda device comfig
	else if (!strncmp(p, "bel", 3)) {
		confSetPriority = pt_BELOWNORMAL;
    } else if (!strncmp(p, "nor", 3)) {
		confSetPriority = pt_NORMAL;
    } else if (!strncmp(p, "abo", 3)) {
