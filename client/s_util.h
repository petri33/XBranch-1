...
#ifdef _WIN32
//Jason: Safe exit check macr0 to play nicer with Cuda
	#ifdef USE_CUDA  
		#define SAFE_EXIT_CHECK  do { \
			if (worker_thread_exit_request) { \
			   fprintf(stderr,"-> Worker received exit request, syncing Cuda..."); CUDASYNC; fprintf(stderr,"Done.\n"); \
		...
		#define SAFE_EXIT_CHECK  do { \
			if (worker_thread_exit_request) { \
			   fprintf(stderr,"  Worker Acknowledging exit request, spinning-> "); worker_thread_exit_ack = true; \
			   while (1) Sleep(10); \
			} \
		} while (0);
	#endif
...
extern unsigned int pow2(unsigned int num);
...
#if defined(_WIN32)
   // Needed for sync & free functionailty on Windows, for safe exit.
...
// macro so we can get file and line info.
	...
	#define SETIERROR( err, errmsg ) do {																\
		if (gSetiUseCudaDevice) { CUDASYNC; cudaAcc_free(); fprintf(stderr,"Cuda sync'd & freed.\n"); } \
			fprintf(stderr,"Preemptively acknowledging a safe Exit. ->\n");								\
			fflush(NULL);																				\
			worker_thread_exit_ack = true;																\
