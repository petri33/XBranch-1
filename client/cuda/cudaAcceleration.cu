...
/* CUresult status = cuInit(0);
     ...
     status = cuCtxCreate( &hcuContext, 0x4, hcuDevice ); //0x4 is CU_CTX_BLOCKING_SYNC
     if(status != CUDA_SUCCESS)
        return false;*/
		
#if CUDART_VERSION < 4000
	 CUDA_ACC_SAFE_CALL(cudaSetDeviceFlags(cudaDeviceBlockingSync),false);
...
{	
    ...
    cudaAcc_init++;
    CUDA_ACC_SAFE_CALL(cudaMemcpy(dev_cx_DataArray, cx_DataArray, NumDataPoints * sizeof(*cx_DataArray), cudaMemcpyHostToDevice),true);
	CUDA_ACC_SAFE_CALL((CUDASYNC),true);

	cu_err = cudaMalloc((void**) &dev_cx_ChirpDataArray, sizeof(*dev_cx_ChirpDataArray) * (NumDataPoints*PADVAL));    
    if( cudaSuccess != cu_err) 
    {
    ...
    dev_TripletResults = dev_GaussFitResults; // Triplet results in the SAME PLACE as dev_GaussFitResults
    dev_PulseResults = dev_GaussFitResults; // Triplet results in the SAME PLACE as dev_GaussFitResults
    ...
    TripletResults = GaussFitResults;
    PulseResults = GaussFitResults;    
    ...
    dev_tmp_pot = (float*) dev_PoTPrefixSum;
...
//    cudaError_t cu_err;
	...
	int ac_size = sizeof(*dev_AutoCorrIn)*ac_fftlen*4;

	if( gCudaAutocorrelation )
	{
		dev_AutoCorrIn = (float2 *) dev_GaussFitResults;
		fprintf(stderr,"re-using dev_GaussFitResults array for dev_AutoCorrIn, %d bytes\n",ac_size);
	}
	if ( gCudaAutocorrelation )
	{
		dev_AutoCorrOut = &dev_AutoCorrIn[ac_fftlen*4];
	...
	blockSums = (float3 *) malloc_a(1024*sizeof(float3),MEM_ALIGN);
