...
#elif defined(HAVE_MACH_ABSOLUTE_TIME) && defined(HAVE_MACH_MACH_TIME_H)
...
tick_t hires_timer::ticks() {
  if (use_fallback) {
    ...
    if (t < last_ticks) {
       rollover+=(static_cast<tick_t>(1)<<32);   // unfortunately a rollover returns a
                                   // bogus elapsed time.
    }
    ...
    tick_t t=get_ticks();
    if (t < last_ticks) {
       rollover+=(static_cast<tick_t>(1)<<32);   
    }
