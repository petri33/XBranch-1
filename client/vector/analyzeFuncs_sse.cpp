...
inline void v_vsubTranspose4(float *in, float *out, int xline, int yline) {
    ...
    // not critical, the shuffle-only _MM_TRANSPOSE4_PS isn't optimal
...
int v_vpfTranspose8x4ntw(int x, int y, float *in, float *out) {
    ...
    for (j=0;j<y-3;j+=4) {
        for (i=0;i<x-7;i+=8) {
            ...
            _mm_prefetch(tin+0*x+10,_MM_HINT_T0);
            _mm_prefetch(tin+1*x+10,_MM_HINT_T0);
            _mm_prefetch(tin+2*x+10,_MM_HINT_T0);
            _mm_prefetch(tin+3*x+10,_MM_HINT_T0);
        }
        for (;i<x;i++) {
            out[i*y+j]=in[j*x+i];
        }
...
int v_vGetPowerSpectrum(
    ...
    FORCE_FRAME_POINTER;
    BOINCASSERT(((long)PowerSpectrum & 0xf) == 0);
    BOINCASSERT(((long)FreqData & 0xf) == 0);
...
//    ALIGNED(float, 16) v_angle[4], v_angle2[4];
        ...
        __asm {
            ...
            fld roundVal
            fld angles[0 * 8]  // get angle double
            fadd st(0), st(1)  // + roundVal
            fsub st(0), st(1)  // - roundVal
            fsubr angles[0 * 8]  // angle - round(angle)
            fstp v_angle[0 * 4]  // store as float
            fld angles[1 * 8]
            fadd st(0), st(1)
            fsub st(0), st(1)
            fsubr angles[1 * 8]
...
__asm__ __volatile__ ("fldcw %0" : : "m" (fpucw2));
        v_angle[0] = angles[0] - ((angles[0] + roundVal) - roundVal);
        v_angle[1] = angles[1] - ((angles[1] + roundVal) - roundVal);
        v_angle[2] = angles[2] - ((angles[2] + roundVal) - roundVal);
        v_angle[3] = angles[3] - ((angles[3] + roundVal) - roundVal);
        v_angle2[0] = angles[4] - ((angles[4] + roundVal) - roundVal);
        v_angle2[1] = angles[5] - ((angles[5] + roundVal) - roundVal);
