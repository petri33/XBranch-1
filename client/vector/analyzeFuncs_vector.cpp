...
BaseLineSmooth_func ChooseBaseLineSmooth() {
    ...
    for (i=0;i<NumDataPoints;i++) {
        indata[i][0]=((rand()&RAND_MAX)>(RAND_MAX/2))?(float)-1:(float)1;
        indata[i][1]=((rand()&RAND_MAX)>(RAND_MAX/2))?(float)-1:(float)1;
    }
...
ChirpData_func ChooseChirpData() {
    ...
    for (i=0;i<NumDataPoints;i++) {
        indata[i][0]=(float)((rand()&RAND_MAX)>(RAND_MAX/2))?(float)-1:(float)1;
        indata[i][1]=(float)((rand()&RAND_MAX)>(RAND_MAX/2))?(float)-1:(float)1;
    }
#if !defined(USE_ASMLIB) && !defined(__APPLE_CC__)
    install_sighandler();
    // for (i=0;i<k;i++) {
    for (i=0;i<1;i++) {     // TODO: CHANGED ONLY FOR TESTING PURPOSES, DELETE THAT LATER ON !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        if (!setjmp(jb)) {
#else
    for (i=0;i<k;i++) {
#endif
