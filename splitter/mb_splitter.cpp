              ...
              if (!strncmp(argv[i],"-atnight",MAX(strlen(argv[i]),7))) {
                *atnight=1;
			    ...
			    while (isgraph(*(fe++))) trigger_file_path[fe-ep-2]=*fe ;
                          } else if (!strncmp(argv[i],"-alfa",MAX(ep-argv[i],2))) {
...
int main(int argc, char *argv[]) {
  ...
  waveform_filter<complex<signed char> > blanker_filter;
  ...
  log_messages.printf(SCHED_MSG_LOG::MSG_DEBUG, "settings: \n\n%s\n", splitter_settings.print_xml(1,1,0).c_str());
	...
	double rot=i->header.scram_alfashm.AlfaMotorPosition;
	double za=i->header.scram_agc.Za;
	double az=i->header.scram_agc.Az;
	coordinate_t tmp;

	tmp.time=(i->header.coord_time.jd().uval());
	double unix_sec=seconds(i->header.coord_time-seti_time((time_t)0)).uval();
	co_ZenAzCorrection(tel,&za,&az,rot);
	seti_AzZaToRaDec(az,za,unix_sec,tmp.ra,tmp.dec);
	// fprintf(stderr,"%f %f %f %f %f %f %f\n",unix_sec,i->header.scram_agc.Az,i->header.scram_agc.Za,az,za,tmp.ra,tmp.dec);
         
        coord_history[i->header.coord_time]=tmp;
      }
