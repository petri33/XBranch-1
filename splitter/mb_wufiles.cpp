...
tel, std::vector<workunit> &wuheader) {
  ...
  workunit_grp wugrp;
  sprintf(wugrp.name,"%s.%ld.%d.%ld.%d",
...
void rename_wu_files() {
  ...
  for (i=0;i<NSTRIPS;i++) {
	...
	if ( ar <= beam ) {
	  db_wu.rsc_fpops_est=8.036e+13;
	} else if ( ar <= ( dur*min_slew ) ) {
          db_wu.rsc_fpops_est=4.805e+13+2.296e+12/ar;
        } else if ( ar <= ( dur*max_slew ) ) {
          db_wu.rsc_fpops_est=4.592e+13+1.476e+13/ar;
        } else {
          db_wu.rsc_fpops_est=2.378e+13;
        }
