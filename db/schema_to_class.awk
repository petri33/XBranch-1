...
/;/            {
		  ...
		  for (i=1;i<nfields;i++) {
		    print "\t  {  " >sourcefile
		    if (type[i]=="r" || type[i]=="t") {
		      print "\t  "fields[i]".parse(SQL_ROW(s["i-1"]->c_str(),0));" >sourcefile
		    } else if (index(type[i],"v")==1) {
		      n=split(type[i],a," ")
		      stype=a[n]
		      n=split(stype,a,"(")
		      ...
		      print "\t    "fields[i]".clear();" >sourcefile
		      print "\t      SQL_ROW tmp((*s["i-1"]).c_str()); ">sourcefile
		      if (sis_deftype) {
		        print "\t      for (i=0;i<tmp.argc();i++) {" >sourcefile
			print "\t "fields[i]".push_back("stype"(SQL_ROW((*tmp[i]).c_str())));" >sourcefile
		      } else {          
		        print "\t        for (i=0;i<tmp.argc();i++) {" >sourcefile
		  ...
		  print "\t      SQL_ROW row(s.c_str(),"nfields-1");" >sourcefile
